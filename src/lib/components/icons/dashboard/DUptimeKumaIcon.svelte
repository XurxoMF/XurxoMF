<script lang="ts">
	import type { Without } from '$lib/utils';

	import BaseIcon, { type BaseIconProps } from '../BaseIcon.svelte';

	let { ...restProps }: Without<BaseIconProps, 'children'> = $props();
</script>

<BaseIcon {...restProps}>
	<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 622 622" fill="none" class="h-[1em] w-[1em]">
		<g transform="translate(320 320)">
			<linearGradient
				id="a"
				x1="-82.404"
				x2="121.666"
				y1="38.077"
				y2="-157.263"
				gradientTransform="matrix(0 0 0 -1 .001 -16)"
				gradientUnits="userSpaceOnUse"
			>
				<stop offset="0" style="stop-color:#5cdd8b" />
				<stop offset="1" style="stop-color:#86e6a9" />
			</linearGradient>
			<path
				d="M161.4-93.4c53.7 122.7 53.7 199.7 0 230.9-80.5 46.7-290.4 61-350.9-10.9-40.3-47.9-40.3-121.2 0-220 41-67.5 99.2-101.2 174.6-101.2 75.5 0 134.3 33.8 176.3 101.2z"
				style="fill:url(#a);stroke:#f2f2f2;stroke-width:200;stroke-opacity:.51"
			/>
		</g>
	</svg>
</BaseIcon>
